...
def wait_on_checkpoint_download(path, size, total_size, timeout):
    ...
    if download_failed:
        raise TimeoutError("Timeout ({t}s): Checkpoint file incomplete or corrupted.".format(t=timeout))
...
def download_checkpoint(url, path, timeout=60):
    ...
    if not os.path.exists(dir):
        os.mkdir(dir)
    logging.info("Downloading checkpoint...")
...
class Sam():
    def __init__(self, checkpoint_url, checkpoint_path, model_type, device):
        ...
        self.device = device
        logging.warning("create SAM NEWWW")
